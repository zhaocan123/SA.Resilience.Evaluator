...
def getfunc(oldproject, newproject):
    oldinfo = oldproject['funcInfo']
    olddata = dict()
    newinfo = newproject['funcInfo']
    newdata = dict()
    ...
    functionlevel = {}
    paramNum = checkparameter(olddata, newdata, commonlist)
    functionlevel['paramNum'] = {}
    add_functiondata(functionlevel['paramNum'], paramNum)
    functionlevel['variableNum'] = {}
    varNum = checkvar(olddata, newdata, commonlist)
    add_functiondata(functionlevel['variableNum'], varNum)
    functionlevel['cyclomaticComplexity'] = {}
    cycNum = checkcyclComplexity(olddata, newdata, commonlist)
    add_functiondata(functionlevel['cyclomaticComplexity'], cycNum)
...
def checkparameter(olddata, newdata, commonlist):
    '''
    参数个数
    :param olddata:
    :param newdata:
    ...
    droplist = []
    for i in commonlist:
        old_param_num = len(olddata[i]['paramList'])
        new_param_num = len(newdata[i]['paramList'])
...
def checkvar(olddata, newdata, commonlist):
    ...
    droplist = []
    for i in commonlist:
        old_param_num = float(olddata[i]['variableNum'])
        new_param_num = float(newdata[i]['variableNum'])
        if old_param_num < new_param_num:
            funcpath = olddata[i]['locateFile'] + "/" + olddata[i]['name']
            changelist.append({"functionPath": funcpath,
                               "selectedValue": old_param_num,
...
def checkcyclComplexity(olddata, newdata, commonlist):
    '''
    圈复杂度
    :param olddata:
    :param newdata:
    ...
    droplist = []
    for i in commonlist:
        old_param_num = float(olddata[i]['cyclComplexity'])
        new_param_num = float(newdata[i]['cyclComplexity'])
        if old_param_num < new_param_num:
            funcpath = olddata[i]['locateFile'] + "/" + olddata[i]['name']
            changelist.append({"functionPath": funcpath,
                               "selectedValue": old_param_num,
...
if __name__ == '__main__':
    ...
    import json
    f = open('code/data/funcInfo.json', 'r')
    content = f.read()
    ...
    newdata = {"funcInfo": a}
    f.close()
    result = getfunc(olddata, newdata)
    data = json.dumps(result, indent=4, ensure_ascii=False)
    with open('code/pre/funcinfo.json', 'w', newline='\n') as f:
        f.write(data)
