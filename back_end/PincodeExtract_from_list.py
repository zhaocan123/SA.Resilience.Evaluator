...
def fcr1g_cd_A(require_document_list, use_case_document_list):
    ...
    if require_document_list and use_case_document_list:
        ...
        for require_node in require_document_list:
            if '功能' in require_node.category and require_node.associated_use_case_ID:
                isfailure = False
...
def fcp1g_cd_B(require_document_list):
    ...
    if require_document_list:
        ...
        count = 0
        for require_node in require_document_list:
            if '功能' in require_node.category  and require_node.its_fathers == []:
                count = count + 1
    else:
        count = None
    if current_document_name != '':
...
def fcp1g_cd_A(require_document_list):
    ...
    if require_document_list:
        ...
        count = 0
        for require_node in require_document_list:
            if '功能' in require_node.category  and require_node.its_fathers == [] and require_node.current_state != '已关闭':
                count = count + 1
    else:
        count = None
    if current_document_name != '':
...
def fap1g_cd_B(require_document_list):
    ...
    if require_document_list:
        ...
        for require_node in require_document_list:
            if '功能' in require_node.category  and require_node.specific_require:
...
def ptb1g_cd_Ai(require_document_list, use_case_document_list):
    ...
    for require_node in require_document_list:
        current_document_name = use_case_document_list[0].document_name
        if '性能' in require_node.category  and require_node.require_description.find('响应时间') != -1:
...
def template_threshold_etc(require_document_list, category_input, description_kind):
    ...
    if require_document_list:
        ...
        development_value = ''
        for require_node in require_document_list:
            if category_input in require_node.category  and require_node.require_description.find(
                    description_kind) != -1:
...
def template_extract_situation_num(require_document_list, use_case_document_list, category_input, require_kind, find_what):  # 从实际情况中提取数字
    ...
    if require_document_list and use_case_document_list:
        ...
        need_in_situation = []
        for require_node in require_document_list:
            if category_input in require_node.category  and require_node.require_description.find(require_kind) != -1:
...
def pca3s_cd_A(require_document_list, use_case_document_list):
    need_in_situation = template_extract_situation_num(require_document_list, use_case_document_list, '性能', '我希望系统用户访问增长', '新增用户数')
    for each_one in need_in_situation:
...
def pca3s_cd_B(require_document_list, use_case_document_list):
    need_in_situation = template_extract_situation_num(require_document_list, use_case_document_list, '性能', '我希望系统用户访问增长', '观察时长')
    for each_one in need_in_situation:
        each_one['des'] = '第' + str(int(each_one['id']) + 1) + '次观察中的观察时间'
        each_one['id'] = 'B' + each_one['id']
    return need_in_situation
...
def template_extract_situation_list_len(require_document_list, category_input, description_kind):  # 数据项列表长度
    ...
    for require_node in require_document_list:
        current_document_name = require_document_list[0].document_name
        if category_input in require_node.category  and require_node.require_description.find(description_kind) != -1:
            current_id = require_node.id
            current_des = template_what_i_hope(require_node.require_description)
            require_data_list = get_data_from_description(require_node.require_description)
            break
...
def cco1g_cd_A(require_document_list, use_case_document_list):
    L = template_extract_situation_list(require_document_list, use_case_document_list, '兼容性', '与以下软件共享运行环境')
    L['id'] = 'A'
...
def cco1g_cd_B(require_document_list):
    L = template_extract_situation_list_len(require_document_list, '兼容性', '与以下软件共享运行环境')
    L['id'] = 'B'
...
def cin1g_cd_A(require_document_list, use_case_document_list):
    L = template_extract_situation_list(require_document_list, use_case_document_list, '兼容性', '以下数据格式交换数据')
    L['id'] = 'A'
...
def cin1g_cd_B(require_document_list):
    L = template_extract_situation_list_len(require_document_list, '兼容性', '以下数据格式交换数据')
    L['id'] = 'B'
    L['des'] = '需要交换数据格式的数量'
    return L
...
def cin2g_cd_A(require_document_list, use_case_document_list):
    L = template_extract_situation_list(require_document_list, use_case_document_list, '兼容性', '以下协议交换数据')
    L['id'] = 'A'
    L['des'] = '实际支持数据交换协议的数量'
    return L
...
def cin2g_cd_B(require_document_list):
    L = template_extract_situation_list_len(require_document_list, '兼容性', '以下协议交换数据')
    L['id'] = 'B'
    L['des'] = '规定支持的数据交换协议数量'
    return L
...
def cin3s_cd_A(require_document_list, use_case_document_list):
    L = template_extract_situation_list(require_document_list, use_case_document_list, '兼容性', '支持以下外部接口')
    L['id'] = 'A'
    L['des'] = '有效的外部接口数量'
    return L
...
def cin3s_cd_B(require_document_list):
    L = template_extract_situation_list_len(require_document_list, '兼容性', '支持以下外部接口')
    L['id'] = 'B'
    L['des'] = '规定的外部接口数量'
    return L
...
def uap1g_cd_A(require_document_list, use_case_document_list):
    L = template_extract_situation_list(require_document_list, use_case_document_list, '易用性', '以下场景中系统使用方法')
    L['id'] = 'A'
...
def uap1g_cd_B(require_document_list):
    L = template_extract_situation_list_len(require_document_list, '易用性', '以下场景中系统使用方法')
    L['id'] = 'B'
    L['des'] = '产品的使用场景数量'
    return L
...
def uap2s_cd_A(require_document_list, use_case_document_list):
    L = template_extract_situation_list(require_document_list, use_case_document_list, '易用性', '以下任务提供演示功能')
    L['id'] = 'A'
    L['des'] = '具有演示功能的任务的数量'
    return L
...
def uap2s_cd_B(require_document_list):
    L = template_extract_situation_list_len(require_document_list, '易用性', '以下任务提供演示功能')
    L['id'] = 'B'
...
def uap3s_cd_A(require_document_list, use_case_document_list):
    need_in_situation = template_extract_situation_num(require_document_list, use_case_document_list, '易用性', '系统网站的引导页数量', '引导页数')
    for each_one in need_in_situation:
...
def uap3s_cd_B(require_document_list):
    threshold, template, development_value = template_threshold_etc(require_document_list, '易用性', '系统网站的引导页数量')
    threshold['id'] = 'B'
...
def ule1g_cd_A(require_document_list, use_case_document_list):
    L = template_extract_situation_list(require_document_list, use_case_document_list, '易用性', '用户文档和帮助机制中能描述以下功能')
    L['id'] = 'A'
...
def ule1g_cd_B(require_document_list):
    ...
    for require_node in require_document_list:
        current_document_name = require_document_list[0].document_name
        if '功能' in require_node.category  :
            L = L + 1
    if require_document_list == []:
        L = None
    if current_document_name != '':
...
def ule2s_cd_A(require_document_list, use_case_document_list):
    L = template_extract_situation_list(require_document_list, use_case_document_list, '易用性', '以下输入字段能自动填充默认值')
    L['id'] = 'A'
...
def ule2s_cd_B(require_document_list):
    L = template_extract_situation_list_len(require_document_list, '易用性', '以下输入字段能自动填充默认值')
    L['id'] = 'B'
    L['des'] = '具有默认值的输入字段的数量'
    return L
...
def ule3s_cd_A(require_document_list, use_case_document_list):
    L = template_extract_situation_list(require_document_list, use_case_document_list, '易用性', '提供出错原因和可能解决方法')
    L['id'] = 'A'
...
def ule3s_cd_B(require_document_list):
    L = template_extract_situation_list_len(require_document_list, '易用性', '提供出错原因和可能解决方法')
    L['id'] = 'B'
    L['des'] = '差错信息的数量'
    return L
...
def ule4s_cd_A(require_document_list, use_case_document_list):
    need_in_situation = template_extract_situation_num(require_document_list, use_case_document_list, '易用性', '完成常规任务所需信息元素和步骤的数量', '用户可理解的信息元素和步骤')
    for each_one in need_in_situation:
...
def ule4s_cd_B(require_document_list):
    threshold, template, development_value = template_threshold_etc(require_document_list, '易用性', '完成常规任务所需信息元素和步骤的数量')
    threshold['id'] = 'B'
...
def uop1g_cd_A(require_document_list, use_case_document_list):
    L = template_extract_situation_list(require_document_list, use_case_document_list, '易用性', '操作结果与外观具有一致性')
    L['id'] = 'A'
    L['des'] = '不一致的特定交互式任务数量'
    return L
...
def uop1g_cd_B(require_document_list):
    L = template_extract_situation_list_len(require_document_list, '易用性', '操作结果与外观具有一致性')
    L['id'] = 'B'
    L['des'] = '需要一致的交互任务的数量'
    return L
...
def uop2g_cd_A(require_document_list, use_case_document_list):
    n = template_use_case_pass(require_document_list, use_case_document_list, '易用性', '给用户传达正确结果或指令')
    n['id'] = 'A'
...
def uop2g_cd_B(require_document_list):
    n = template_associated_use_case_num(require_document_list, '易用性', '给用户传达正确结果或指令')
    n['id'] = 'B'
    n['des'] = '实现的消息数量'
    return n
...
def uop3s_cd_A(require_document_list, use_case_document_list):
    L = template_extract_situation_list(require_document_list, use_case_document_list, '易用性', '功能和操作规程支持用户定制')
    L['id'] = 'A'
...
def uop3s_cd_B(require_document_list):
    L = template_extract_situation_list_len(require_document_list, '易用性', '功能和操作规程支持用户定制')
    L['id'] = 'B'
...
def uop4s_cd_A(require_document_list, use_case_document_list):
    L = template_extract_situation_list(require_document_list, use_case_document_list, '易用性', '用户界面元素支持用户定制')
    L['id'] = 'A'
...
def uop4s_cd_B(require_document_list):
    L = template_extract_situation_list_len(require_document_list, '易用性', '用户界面元素支持用户定制')
    L['id'] = 'B'
...
def uop5s_cd_A(require_document_list, use_case_document_list):
    L = template_extract_situation_list(require_document_list, use_case_document_list, '易用性', '能够监视以下功能状态')
    L['id'] = 'A'
    L['des'] = '具有状态监视能力的功能数量'
    return L
...
def uop5s_cd_B(require_document_list):
    L = template_extract_situation_list_len(require_document_list, '易用性', '能够监视以下功能状态')
    L['id'] = 'B'
...
def uop6s_cd_A(require_document_list, use_case_document_list):
    L = template_extract_situation_list(require_document_list, use_case_document_list, '易用性', '支持撤销操作或重新确认')
    L['id'] = 'A'
...
def uop6s_cd_B(require_document_list):
    L = template_extract_situation_list_len(require_document_list, '易用性', '支持撤销操作或重新确认')
    L['id'] = 'B'
...
def uop7s_cd_A(require_document_list, use_case_document_list):
    L = template_extract_situation_list(require_document_list, use_case_document_list, '易用性', '使用以下信息结构')
    L['id'] = 'A'
...
def uop7s_cd_B(require_document_list):
    L = template_extract_situation_list_len(require_document_list, '易用性', '使用以下信息结构')
    L['id'] = 'B'
    L['des'] = '使用的信息结构数量'
    return L
...
def uop8s_cd_A(require_document_list, use_case_document_list):
    need_in_situation = template_extract_situation_num(require_document_list, use_case_document_list, '易用性', '具有相似项且外观相似的比例', '具有相似项但外观不同的用户界面')
    for each_one in need_in_situation:
...
def uop8s_cd_B(require_document_list, use_case_document_list):
    need_in_situation = template_extract_situation_num(require_document_list, use_case_document_list, '易用性', '具有相似项且外观相似的比例', '具有相似项的用户界面')
    for each_one in need_in_situation:
...
def uop8s_cd_X(require_document_list, use_case_document_list):
    need_in_situation = template_extract_situation_num(require_document_list, use_case_document_list, '易用性', '具有相似项且外观相似的比例', '具有相似项且外观相似')
    for each_one in need_in_situation:
...
def uop9s_cd_A(require_document_list, use_case_document_list):
    L = template_extract_situation_list(require_document_list, use_case_document_list, '易用性', '通过键盘、鼠标或语音等方法启动以下任务')
    L['id'] = 'A'
...
def uop9s_cd_B(require_document_list):
    L = template_extract_situation_list_len(require_document_list, '易用性', '通过键盘、鼠标或语音等方法启动以下任务')
    L['id'] = 'B'
    L['des'] = '系统支持的任务数量'
    return L
...
def uep1g_cd_A(require_document_list, use_case_document_list):
    L = template_extract_situation_list(require_document_list, use_case_document_list, '易用性', '以下用户操作时抵御误操作')
    L['id'] = 'A'
...
def uep1g_cd_B(require_document_list):
    L = template_extract_situation_list_len(require_document_list, '易用性', '以下用户操作时抵御误操作')
    L['id'] = 'B'
...
def uep2s_cd_A(require_document_list, use_case_document_list):
    L = template_extract_situation_list(require_document_list, use_case_document_list, '易用性', '检测到以下输入差错时提示纠正')
    L['id'] = 'A'
...
def uep2s_cd_B(require_document_list):
    L = template_extract_situation_list_len(require_document_list, '易用性', '检测到以下输入差错时提示纠正')
    L['id'] = 'B'
    L['des'] = '检测到的输入差错数量'
    return L
...
def uep3s_cd_A(require_document_list, use_case_document_list):  # 需求应该包含子需求
    ...
    for require_node in require_document_list:
        current_document_name = require_document_list[0].document_name
        if '易用性' in require_node.category and require_node.require_description.find('检测用户操作的差错可以进行纠正') != -1:
            current_id = require_node.id
            current_des = template_what_i_hope(require_node.require_description)
            if require_node.associated_use_case_ID:
                for use_case_ID in require_node.associated_use_case_ID:
...
def uin1s_cd_A(require_document_list, use_case_document_list):  # 需求应该包含子需求
    ...
    for require_node in require_document_list:
        current_document_name = require_document_list[0].document_name
        if '易用性' in require_node.category  and require_node.require_description.find('显示界面外观舒适可以令人愉悦') != -1:
            current_id = require_node.id
            current_des = template_what_i_hope(require_node.require_description)
            if require_node.associated_use_case_ID:
                for use_case_ID in require_node.associated_use_case_ID:
...
def uac1g_cd_A(require_document_list, use_case_document_list):
    L = template_extract_situation_list(require_document_list, use_case_document_list, '易用性', '以下功能可以支持特殊群体使用')
    L['id'] = 'A'
...
def uac1g_cd_B(require_document_list):
    L = template_extract_situation_list_len(require_document_list, '易用性', '以下功能可以支持特殊群体使用')
    L['id'] = 'B'
    L['des'] = '实现的功能数量'
    return L
...
def uac2s_cd_A(require_document_list, use_case_document_list):
    L = template_extract_situation_list(require_document_list, use_case_document_list, '易用性', '支持以下语种')
    L['id'] = 'A'
    L['des'] = '实际支持的语种数量'
    return L
...
def uac2s_cd_B(require_document_list):
    L = template_extract_situation_list_len(require_document_list, '易用性', '支持以下语种')
    L['id'] = 'B'
    L['des'] = '需要支持的语种数量'
    return L
...
def rma1g_cd_A(require_document_list, bug_document_list):
    ...
    n = 0
    for require_node in require_document_list:
        if '可靠性' in require_node.category  and require_node.require_description.find('可靠性相关的故障修复率') != -1:
            current_id = require_node.id
            current_des = template_what_i_hope(require_node.require_description)
...
def rma2g_cd_A(require_document_list, use_case_document_list):
    need_in_situation = template_extract_situation_num(require_document_list, use_case_document_list, '可靠性', '平均失效间隔时间', '运行时间')
    if need_in_situation == []:
        need_in_situation = template_extract_situation_num(require_document_list, use_case_document_list, '可靠性',
                                                           '平均失效间隔时间', '观察周期时长')
    for each_one in need_in_situation:
...
def rma2g_cd_B(require_document_list, use_case_document_list):
    need_in_situation = template_extract_situation_num(require_document_list, use_case_document_list, '可靠性', '平均失效间隔时间', '失效次数')
    if need_in_situation == []:
        need_in_situation = template_extract_situation_num(require_document_list, use_case_document_list, '可靠性',
                                                           '平均失效间隔时间', '失效数量')
    for each_one in need_in_situation:
...
def rma2g_cd_X(require_document_list, use_case_document_list):
    need_in_situation = template_extract_situation_num(require_document_list, use_case_document_list, '可靠性', '平均失效间隔时间', '平均失效间隔')
    for each_one in need_in_situation:
...
def rma3g_cd_A(require_document_list, use_case_document_list):
    need_in_situation = template_extract_situation_num(require_document_list, use_case_document_list, '可靠性', '周期失效率', '失效数量')
    for each_one in need_in_situation:
...
def rma3g_cd_B(require_document_list, use_case_document_list):
    need_in_situation = template_extract_situation_num(require_document_list, use_case_document_list, '可靠性', '周期失效率', '观察周期')
    for each_one in need_in_situation:
...
def rma3g_cd_X(require_document_list, use_case_document_list):
    need_in_situation = template_extract_situation_num(require_document_list, use_case_document_list, '可靠性', '周期失效率', '周期失效率')
    for each_one in need_in_situation:
...
def rav1g_cd_A(require_document_list, use_case_document_list):
    need_in_situation = template_extract_situation_num(require_document_list, use_case_document_list, '可靠性', '内运行时间', '实际运行时间')
    for each_one in need_in_situation:
...
def rav1g_cd_B(require_document_list):
    threshold, template, development_value = template_threshold_etc(require_document_list, '可靠性', '内运行时间')
    threshold['id'] = 'B'
...
def rav2g_cd_A(require_document_list, use_case_document_list):
    need_in_situation = template_extract_situation_num(require_document_list, use_case_document_list, '可靠性', '平均宕机时间', '总的宕机时间')
    for each_one in need_in_situation:
...
def rav2g_cd_B(require_document_list, use_case_document_list):
    need_in_situation = template_extract_situation_num(require_document_list, use_case_document_list, '可靠性', '平均宕机时间', '宕机的次数')
    for each_one in need_in_situation:
...
def rav2g_cd_X(require_document_list, use_case_document_list):
    need_in_situation = template_extract_situation_num(require_document_list, use_case_document_list, '可靠性', '平均宕机时间', '平均宕机时间')
    for each_one in need_in_situation:
...
def rft1g_cd_A(require_document_list, use_case_document_list):
    n = template_use_case_pass(require_document_list, use_case_document_list, '可靠性', '能控制的故障模式数量')
    n['id'] = 'A'
...
def rft1g_cd_B(require_document_list):
    n = template_associated_use_case_num(require_document_list, '可靠性', '能控制的故障模式数量')
    n['id'] = 'B'
...
def rft3s_cd_Ai(require_document_list, use_case_document_list):
    need_in_situation = template_extract_situation_num(require_document_list, use_case_document_list, '可靠性', '发生故障到报告故障的时间间隔', '报告时刻')
    for each_one in need_in_situation:
...
def rft3s_cd_Bi(require_document_list, use_case_document_list):
    need_in_situation = template_extract_situation_num(require_document_list, use_case_document_list, '可靠性', '发生故障到报告故障的时间间隔', '故障被检测')
    for each_one in need_in_situation:
...
def rft3s_cd_Xi(require_document_list, use_case_document_list):
    need_in_situation = template_extract_situation_num(require_document_list, use_case_document_list, '可靠性', '发生故障到报告故障的时间间隔', '时间间隔')
    for each_one in need_in_situation:
...
def rre1g_cd_Ai(require_document_list, use_case_document_list):
    need_in_situation = template_extract_situation_num(require_document_list, use_case_document_list, '可靠性', '从失效中恢复所需时间', '花费的总时间')
    for each_one in need_in_situation:
...
def rre2s_cd_A(require_document_list, use_case_document_list):
    L = template_extract_situation_list(require_document_list, use_case_document_list, '可靠性', '以下数据项定期备份')
    L['id'] = 'A'
    L['des'] = '实际定期备份数据项的数量'
    return L
...
def rre2s_cd_B(require_document_list):
    L = template_extract_situation_list_len(require_document_list, '可靠性', '以下数据项定期备份')
    L['id'] = 'B'
    L['des'] = '需要备份的数据项的数量'
    return L
...
def sac1g_cd_B(require_document_list, use_case_document_list):
    ...
    if require_document_list and use_case_document_list:
        ...
        need_in_situation = []
        for require_node in require_document_list:
            if '安全' in require_node.category  and require_node.require_description.find('对系统或数据的访问以日志形式记录') != -1:
                current_id = require_node.id
                current_des = template_what_i_hope(require_node.require_description)
                if require_node.associated_use_case_ID:
                    for use_case_ID in require_node.associated_use_case_ID:
...
def mre2s_cd_A(require_document_list, use_case_document_list):
    L = template_extract_situation_list(require_document_list, use_case_document_list, '可维护性', '模块的实现遵循以下编码规则')
    L['id'] = 'A'
...
def mmo2s_cd_Xi(require_document_list):
    threshold, template, development_value = template_threshold_etc(require_document_list, '可维护性', '模块的圈复杂度得分')
    threshold['id'] = 'Xi'
    threshold['des'] = '模块要求的圈复杂度得分'
    return threshold, template, development_value

def man1g_cd_A(require_document_list, use_case_document_list):
    need_in_situation = template_extract_situation_num(require_document_list, use_case_document_list, '可维护性', '操作期间记录审计追踪所需的日志', '日志条数')
    sum = 0
    if need_in_situation:
        for each_one in need_in_situation:
            sum += 1
...
def man1g_cd_B(require_document_list, use_case_document_list):
    ...
    for require_node in require_document_list:
        current_document_name = require_document_list[0].document_name
        if '可维护性' in require_node.category  and require_node.require_description.find('操作期间记录审计追踪所需的日志') != -1:
            current_id = require_node.id
            current_des = template_what_i_hope(require_node.require_description)
            if require_node.associated_use_case_ID:
                for use_case_ID in require_node.associated_use_case_ID:
...
def man2s_cd_A(require_document_list, use_case_document_list):
    n = template_use_case_pass(require_document_list, use_case_document_list, '可维护性', '实现的诊断功能可以生成有效的原因分析')
    n['id'] = 'A'
...
def man2s_cd_B(require_document_list):
    n = template_associated_use_case_num(require_document_list, '可维护性', '实现的诊断功能可以生成有效的原因分析')
    n['id'] = 'B'
    n['des'] = '已实现的诊断功能数量'
    return n
...
def man3s_cd_A(require_document_list, use_case_document_list):
    L = template_extract_situation_list(require_document_list, use_case_document_list, '可维护性', '提供以下诊断功能')
    L['id'] = 'A'
    L['des'] = '已实现的诊断功能数量'
    return L
...
def man3s_cd_B(require_document_list):
    L = template_extract_situation_list_len(require_document_list, '可维护性', '提供以下诊断功能')
    L['id'] = 'B'
    L['des'] = '需要实现的诊断功能数量'
    return L
...
def mmd1g_cd_Ai(require_document_list, use_case_document_list):
    current_document_name = ''
    current_id = ''
    current_des = ''
    actual_data_list = []
    current_id_list = []
    for require_node in require_document_list:
        if '可维护性' in require_node.category  and require_node.require_description.find('对以下项目进行维护升级') != -1:
            current_id = require_node.id
            current_des = template_what_i_hope(require_node.require_description)
            if require_node.its_sons:
                for son_id in require_node.its_sons:
                            ...
                            if son_node.associated_use_case_ID:
                                for use_case_id in son_node.associated_use_case_ID:
                                    for use_case_node in use_case_document_list:
                                        current_document_name = use_case_document_list[0].document_name
                                        if use_case_node.id == use_case_id:
                                            ret = re.findall(".*实际维护([1-9]\d*\.\d+|0\.\d+|0|[1-9]\d*)h",use_case_node.actual_situation)
                                            if ret is not None:
                                                num = float(ret[0])
...
def mmd1g_cd_Bi(require_document_list, use_case_document_list):
    current_document_name = ''
    current_id = ''
    current_des = ''
    actual_data_list = []
                            ...
                            if son_node.associated_use_case_ID:
                                for use_case_id in son_node.associated_use_case_ID:
                                    for use_case_node in use_case_document_list:
                                        current_document_name = use_case_document_list[0].document_name
                                        if use_case_node.id == use_case_id:
                                            ret = re.findall(".*预期维护([1-9]\d*\.\d+|0\.\d+|0|[1-9]\d*)h",use_case_node.expect)
                                            if ret:
                                                num = float(ret[0])
                                                actual_data_list.append(num)
                                                if use_case_node.id not in current_id_list:
                ...
                for son_ID in require_node.its_sons:
                    for son_node in require_document_list:
                        if son_ID == son_node.id:
                            re1 = re.findall(r"我希望在(.+)到(.+)内对以下项目进行维护升级",son_node.require_description)
                            if re1 is not None:
                                close_time = template_get_time_dict(re1[0][1])
...
def mmd3s_cd_B(require_document_list):
    L = template_extract_situation_list_len(require_document_list, '可维护性', '对以下项目进行维护升级')
    L['id'] = 'B'
...
def mte1g_cd_A(require_document_list, use_case_document_list):
    need_in_situation = template_extract_situation_num(require_document_list, use_case_document_list, '可维护性', '系统中测试功能的数量', '已实现的测试功能')
    for each_one in need_in_situation:
...
def mte1g_cd_B(require_document_list):
    threshold, template, development_value = template_threshold_etc(require_document_list, '可维护性', '系统中测试功能的数量')
    threshold['id'] = 'B'
...
def pad1g_cd_A(require_document_list, use_case_document_list):
    ...
    n = 0
    for require_node in require_document_list:
        if '可移植性' in require_node.category  and require_node.require_description.find('以下硬件环境中功能达到要求') != -1:
            current_id = require_node.id
            if require_node.associated_use_case_ID:
                for use_case_ID in require_node.associated_use_case_ID:
                    for use_case_node in use_case_document_list:
...
def pad2g_cd_A(require_document_list, use_case_document_list):
    ...
    n = 0
    for require_node in require_document_list:
        if '可移植性' in require_node.category  and require_node.require_description.find('以下软件环境中功能达到要求') != -1:
            current_id = require_node.id
            if require_node.associated_use_case_ID:
                for use_case_ID in require_node.associated_use_case_ID:
                    for use_case_node in use_case_document_list:
...
def pad3s_cd_A(require_document_list, use_case_document_list):
    ...
    n = 0
    for require_node in require_document_list:
        if '可移植性' in require_node.category  and require_node.require_description.find('以下运营环境中功能达到要求') != -1:
            current_id = require_node.id
            if require_node.associated_use_case_ID:
                for use_case_ID in require_node.associated_use_case_ID:
                    for use_case_node in use_case_document_list:
...
def pin1g_cd_Ai(require_document_list, use_case_document_list):
    need_in_situation = template_extract_situation_num(require_document_list, use_case_document_list, '可移植性', '安装所需时间', '安装消耗的总时间')
    for each_one in need_in_situation:
...
def pin1g_cd_Bi(require_document_list, use_case_document_list):  # 从实际情况中提取数字
    ...
    current_id_list = []
    for require_node in require_document_list:
        if '可移植性' in require_node.category  and require_node.require_description.find('安装所需时间') != -1:
            current_id = require_node.id
            if require_node.associated_use_case_ID:
                for use_case_ID in require_node.associated_use_case_ID:
                    for use_case_node in use_case_document_list:
...
def pin1g_cd_n(require_document_list):
    n = template_associated_use_case_num(require_document_list, '可移植性', '安装所需时间')
    n['id'] = 'n'
    n['des'] = '测量的安装次数'
    return n
...
def pin2g_cd_A(require_document_list, use_case_document_list):
    need_in_situation = template_extract_situation_num(require_document_list, use_case_document_list, '可移植性', '自定义安装规程的数量', '成功自定义的安装规程数')
    for each_one in need_in_situation:
...
def pin2g_cd_B(require_document_list):
    threshold, template, development_value = template_threshold_etc(require_document_list, '可移植性', '自定义安装规程的数量')
    threshold['id'] = 'B'
...
def pre1g_cd_A(require_document_list, use_case_document_list):
    L = template_extract_situation_list(require_document_list, use_case_document_list, '可移植性', '以下用户功能的执行与')
    L['id'] = 'A'
...
def pre1g_cd_B(require_document_list):
    L = template_extract_situation_list_len(require_document_list, '可移植性', '以下用户功能的执行与')
    L['id'] = 'B'
...
def pre2s_cd_A(require_document_list, use_case_document_list):
    L = template_extract_situation_list(require_document_list, use_case_document_list, '可移植性', '以下质量测度优于或等于')
    L['id'] = 'A'
...
def pre2s_cd_B(require_document_list):
    L = template_extract_situation_list_len(require_document_list, '可移植性', '以下质量测度优于或等于')
    L['id'] = 'B'
...
def pre3s_cd_A(require_document_list, use_case_document_list):
    L = template_extract_situation_list(require_document_list, use_case_document_list, '可移植性', '的以下产品功能')
    L['id'] = 'A'
...
def pre3s_cd_B(require_document_list):
    L = template_extract_situation_list_len(require_document_list, '可移植性', '的以下产品功能')
    L['id'] = 'B'
...
def pre4s_cd_A(require_document_list, use_case_document_list):
    L = template_extract_situation_list(require_document_list, use_case_document_list, '可移植性', '一样使用以下数据')
    L['id'] = 'A'
...
def pre4s_cd_B(require_document_list):
    L = template_extract_situation_list_len(require_document_list, '可移植性', '一样使用以下数据')
    L['id'] = 'B'
