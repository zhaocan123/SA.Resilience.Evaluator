...
from Caculate_metric_from_info import *
...
class Require_Node:  # 定义需求节点
    def __init__(self, id, require_name='', require_description='', owning_product='', owning_module='', owning_plan='',
                 source='', close_time='', acceptance_standard='', priority='', current_state='', phase='', category='', segmented_require='', document_name=''):
        # 下面为Node对象增加n个实例变量
        self.id = id  # 自己的ID
...
def data_collation(Path_list):
    ...
    for each_path in Path_list:  # 遍历不同csv文档
        ...
        if set(require_document_keys) <= set(key_list):
            ...
            for each_dict in current_dict:  # 遍历文档中的每一条
                Node = Require_Node(id=each_dict['编号'], require_description=each_dict['描述'],
                                    owning_product=each_dict['所属产品'],
                                    owning_module=each_dict['所属模块'], owning_plan=each_dict['所属计划'],
                                    source=each_dict['来源'], close_time=each_dict['关闭日期'],
                                    acceptance_standard=each_dict['验收标准'], priority=each_dict['优先级'],
                                    current_state=each_dict['当前状态'], document_name=document_name,
                                    phase=each_dict['所处阶段'], category=each_dict['类别'])
        ...
        if set(use_case_document_keys) <= set(key_list):
            print('读取到用例文档')
            for each_dict in current_dict:  # 遍历文档中的每一条
                Node = Use_case_Node(id=each_dict['用例编号'], use_case_name=each_dict['用例标题'],
                                     owning_product=each_dict['所属产品'], precondition=each_dict['前置条件'],
                                     owning_module=each_dict['所属模块'], key_word=each_dict['关键词'],
                                     actual_situation=each_dict['实际情况'], expect=each_dict['预期'], step=each_dict['步骤'],
                                     priority=each_dict['优先级'], use_case_category=each_dict['用例类型'], document_name=document_name,
                                     use_case_state=each_dict['用例状态'], use_case_result=each_dict['结果'])
        ...
        if set(bug_document_keys) <= set(key_list):
            print('读取到bug文档')
            for each_dict in current_dict:  # 遍历文档中的每一条
                Node = Bug_Node(id=each_dict['Bug编号'], owning_product=each_dict['所属产品'],
                                owning_module=each_dict['所属模块'], solution=each_dict['解决方案'],
                                owning_project=each_dict['所属项目'], owning_execute=each_dict['所属执行'],
                                associated_require=each_dict['相关需求'], associated_mission=each_dict['相关任务'],
                                associated_use_case=each_dict['相关用例'], bug_name=each_dict['Bug标题'], document_name=document_name,
                                priority=each_dict['优先级'], bug_state=each_dict['Bug状态'])
...
def caculate_all_metric(require_document_list, use_case_document_list, mission_document_list, bug_document_list):
    ...
    mmd1g = mmd1g_cd(require_document_list, mission_document_list)
...
if __name__ == '__main__':
    ...
    caculate_all_metric(require_document_list, use_case_document_list, mission_document_list, bug_document_list)
    print('')
