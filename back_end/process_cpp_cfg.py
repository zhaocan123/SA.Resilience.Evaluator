...
pwd = os.getcwd()
pwd = pwd.replace("\\", "/")
print("process_cpp_cfg.py: pwd = ", pwd)
...
cpp_class_construtor_re = re.compile("\(CXXConstructExpr, (.*), class (.*)\);")
# cpp_file_io_constructor_re = re.compile("\(CXXConstructExpr, (.*), class (.*)\);")
cpp_constructor_re = re.compile("\(CXXConstructExpr, (.*), (.*)\);")
        ...
        if 'Node' in id:
            ...
            if len(label) > 2:
                ...
                def replace_stmt(str):
                    ...
                    if "(CXXRecordTypedCall" in str:
                        str = str[:str.find("(CXXRecordTypedCall")]
                    if str.find("(CXXConstructExpr") > 1: # 如果是仅有一个参数的类初始化，可能就会有循环标签的问题
                        str = str[:str.find("(CXXConstructExpr")]
                    if '[' + header in str:
                        flag = True
                        count = 0
                        while flag:
                            ...
                            count += 1
                            if count > 5:
                                print(f"replaceing {str} with {flag}")
                                print(result_dict)
                                raise ValueError
                ...
                if value[1] == 0 and (
                    ...
                    if bug_flag == 0:
                        ...
                        if 'T' not in key:
                            ...
                            class_construct = cpp_constructor_re.search(value_str)
                            if class_construct is not None:
                                class_construct = class_construct.group(1)
                                value_str = class_construct
...
if __name__ == "__main__":
    process_cfg(r"test1\graphs\test_class_cpp\unprocessed_main.cfg.dot")
