...
DeclRefExpr* get_DeclRefExpr_const(const MemberExpr* memberExpr)
{
    string type = memberExpr->getType().getAsString();
...
public:
    ...
    void run(const MatchFinder::MatchResult& Result) override
    {
        ...
        if (auto const* classDecl = Result.Nodes.getNodeAs<CXXRecordDecl>("CXXRecordDecl"))
        {
            //cout << "类声明" << endl;
            // 获取类名和类声明位置
            string class_name = classDecl->getNameAsString();
            string loc = classDecl->getSourceRange().getBegin().printToString(Result.Context->getSourceManager());

            string start = classDecl->getSourceRange().getBegin().printToString(Result.Context->getSourceManager());
            start = changes(start);
            start = split(start, ":")[1];
            string end = classDecl->getSourceRange().getEnd().printToString(Result.Context->getSourceManager());
            end = changes(end);
            end = split(end, ":")[1];
            ...
            if (class_name != current_Class.class_name && loc != current_Class.decl_loc)
            {
                for (int i = 0; i < Class_list.size(); i++)
                {
                    if (class_name == Class_list[i].class_name && loc == Class_list[i].decl_loc)
                    {
                        find = 1;
                ...
                if (find == 0) {
                    ...
                    while (base_begin != classDecl->bases_end())
                    {
                        ...
                        string base_decl_loc = (*base_begin).getType()->getAsCXXRecordDecl()->getBeginLoc().printToString(Result.Context->getSourceManager());
                ...
                {
                    string field_name = fields_begin->getCanonicalDecl()->getNameAsString();
                    string field_type = fields_begin->getCanonicalDecl()->getType().getAsString();
                    string type_decl_loc = "";
                    if (fields_begin->getCanonicalDecl()->getType()->getAsCXXRecordDecl() != NULL)
                        type_decl_loc = fields_begin->getCanonicalDecl()->getType()->getAsCXXRecordDecl()->getBeginLoc().printToString(Result.Context->getSourceManager());
                    string field_loc = fields_begin->getCanonicalDecl()->getBeginLoc().printToString(Result.Context->getSourceManager());
                ...
                while (methods_begin != methods_end)
                {
                    ...
                    string isbody = "1";
                    if (!methods_begin->getCanonicalDecl()->hasBody()) //判断函数是否有函数体，仅有一个声明的没有实现的函数停止解析
                    {
                        isbody = "0";
                    }
                    new_method.isbody = isbody;
                    new_method.function_name = methods_begin->getCanonicalDecl()->getNameAsString();
                    new_method.calss = temp_class.class_name;
                    string return_type = methods_begin->getCanonicalDecl()->getReturnType().getAsString();

                    new_method.return_type = return_type;
                    string type_decl_loc = "";
                    if (methods_begin->getCanonicalDecl()->getType()->getAsCXXRecordDecl() != NULL)
                        type_decl_loc = methods_begin->getCanonicalDecl()->getReturnType()->getAsCXXRecordDecl()->getBeginLoc().printToString(Result.Context->getSourceManager());
                    type_decl_loc = changes(type_decl_loc);
                    new_method.return_type = type_decl_loc + "$$" + return_type;
                    string method_decl_loc = methods_begin->getCanonicalDecl()->getBeginLoc().printToString(Result.Context->getSourceManager());
                    method_decl_loc = changes(method_decl_loc);
                    new_method.function_location = method_decl_loc;
                    new_method.file_path = split(method_decl_loc, ":")[0];// + ":" + split(method_decl_loc, ":")[1];
                    string start = methods_begin->getCanonicalDecl()->getSourceRange().getBegin().printToString(Result.Context->getSourceManager());
                    start = changes(start);
                    new_method.start = split(start, ":")[1];
                    string end = methods_begin->getCanonicalDecl()->getSourceRange().getEnd().printToString(Result.Context->getSourceManager());
                    end = changes(end);
                    new_method.end = split(end, ":")[1];
                    ...
                    for (int i = 0; i < params_size; i++)
                    {
                        ...
                        if (params_begin[i]->getType()->getAsCXXRecordDecl() != NULL)
                            type_decl_loc = params_begin[i]->getType()->getAsCXXRecordDecl()->getBeginLoc().printToString(Result.Context->getSourceManager());
                    ...
                    else if (isa<CXXDestructorDecl>(methods_begin->getCanonicalDecl()))
                    {
        ...
        {
            string loc = ifStmt->getBeginLoc().printToString(Result.Context->getSourceManager());
        ...
        if (auto const* functionDecl = Result.Nodes.getNodeAs<FunctionDecl>("FunctiondFeclWithCall"))
        {
            ...
            string loc = functionDecl->getSourceRange().getBegin().printToString(Result.Context->getSourceManager());//获得函数声明的位置
            loc = changes(loc);
            //cout << loc << endl;
            ...
            int find = 0;
            if (functionname != current_Fun.function_name || loc != current_Fun.function_location) {
                for (int i = 0; i < Function_list.size(); i++)
                {
                    if (functionname == Function_list[i].function_name && loc == Function_list[i].function_location)
                    {
                        find = 1;
                ...
                if (find == 0)
                {
                    ...
                    if (f_parameters.size() != 0)
                    {
                        ...
                        for (int i = 0; i < f_parameters.size(); i++)
                        {
                            ...
                            if (param_temp[i]->getType()->getAsCXXRecordDecl() != NULL)
                                type_decl_loc = param_temp[i]->getType()->getAsCXXRecordDecl()->getBeginLoc().printToString(Result.Context->getSourceManager());
                    ...
                    string start = functionDecl->getSourceRange().getBegin().printToString(Result.Context->getSourceManager());
                    start = changes(start);
                    f_temp.start = split(start, ":")[1];
                    string end = functionDecl->getSourceRange().getEnd().printToString(Result.Context->getSourceManager());
                    end = changes(end);
                    f_temp.end = split(end, ":")[1];

                    QualType QT = functionDecl->getReturnType();
                    string type_decl_loc = "";
                    if(functionDecl->getType()->getAsCXXRecordDecl() != NULL)
                        type_decl_loc = functionDecl->getReturnType()->getAsCXXRecordDecl()->getBeginLoc().printToString(Result.Context->getSourceManager());
        ...
        if (auto const* ConstructorDecl = Result.Nodes.getNodeAs<CXXConstructorDecl>("CXXConstructorDecl"))
        {
            // 类构造函数
            //cout << "类构造函数声明" << endl;
            string name = ConstructorDecl->getNameAsString();
            string loc = ConstructorDecl->getBeginLoc().printToString(Result.Context->getSourceManager());
            loc = changes(loc);
            for (int i = 0; i < Function_list.size(); i++)
            {
                if (name == Function_list[i].function_name && loc == Function_list[i].function_location)
                    current_Fun = Function_list[i];
            }
        }

        if (auto const* MethodDecl = Result.Nodes.getNodeAs<CXXMethodDecl>("CXXMethodDecl"))
        {
            // 类成员函数
            //cout << "类成员函数声明" << endl;
            string name = MethodDecl->getNameAsString();
            string loc = MethodDecl->getBeginLoc().printToString(Result.Context->getSourceManager());
            loc = changes(loc);
            for (int i = 0; i < Function_list.size(); i++)
            {
                if (name == Function_list[i].function_name && loc == Function_list[i].function_location)
        ...
        if (auto const* memberExpr = Result.Nodes.getNodeAs<MemberExpr>("MemberExpr"))
        {
            // 获取调用的是函数还是变量
            ...
            if (type == "<bound member function type>" || (type.find("(") != string::npos && type.find(")") != string::npos))
            {
                ...
                string method_decl_loc = memberExpr->getMemberDecl()->getBeginLoc().printToString(Result.Context->getSourceManager());
                method_decl_loc = changes(method_decl_loc);
                string method_call_loc = memberExpr->getBeginLoc().printToString(Result.Context->getSourceManager());
                method_call_loc = changes(method_call_loc);
                string method_return_type = memberExpr->getMemberDecl()->getType().getAsString();
                ...
                {
                    if (fun.call_fun[i].name == method_name && fun.call_fun[i].Decl_Loc == method_decl_loc && fun.call_fun[i].CallExpr_Loc == method_call_loc)
                        find = 1;
                ...
                if (find == 0)
                {
                    for (int i = 0; i < Function_list.size(); i++)
                    {
                        if (current_Fun.function_name == Function_list[i].function_name && current_Fun.function_location == Function_list[i].function_location)
                        {
            ...
            else
            {
                //cout << "aaa" << endl;
                //调用成员变量或成员函数
                string field_type = memberExpr->getMemberDecl()->getType().getAsString();
                string type_decl_loc = "";
                if (memberExpr->getType()->getAsCXXRecordDecl() != NULL)
                    type_decl_loc = memberExpr->getMemberDecl()->getType()->getAsCXXRecordDecl()->getBeginLoc().printToString(Result.Context->getSourceManager());
                string field_name = memberExpr->getMemberDecl()->getNameAsString();
                string field_decl_loc = memberExpr->getMemberDecl()->getBeginLoc().printToString(Result.Context->getSourceManager());
                field_decl_loc = changes(field_decl_loc);
                string field_using_loc = memberExpr->getBeginLoc().printToString(Result.Context->getSourceManager());
                ...
                if (memberExpr->getBase()->getType()->getAsCXXRecordDecl() != NULL)
                    class_decl_loc = memberExpr->getBase()->getType()->getAsCXXRecordDecl()->getBeginLoc().printToString(Result.Context->getSourceManager());
                ...
                {
                    if (cur_class.variables[i].variable_name == field_name && cur_class.variables[i].decl_loc == field_decl_loc && cur_class.variables[i].using_loc == field_using_loc)
                        find = 1;
                ...
                if (find == 0)
                {
                    for (int i = 0; i < Class_list.size(); i++)
                    {
                        if (current_Class.class_name == Class_list[i].class_name && current_Class.decl_loc == Class_list[i].decl_loc)
                        {
        ...
        if (auto const* memberExpr = Result.Nodes.getNodeAs<MemberExpr>("fun_memberExpr"))
        {
            // 获取调用的是函数还是变量
            //cout << "获取类方法调用或类成员变量使用" << endl;
            string type = memberExpr->getType().getAsString();
            ...
            else
            {
                ...
                string field_type = memberExpr->getMemberDecl()->getType().getAsString();
                string type_decl_loc = "";
                if (memberExpr->getMemberDecl()->getType()->getAsCXXRecordDecl() != NULL)
                    type_decl_loc = memberExpr->getMemberDecl()->getType()->getAsCXXRecordDecl()->getBeginLoc().printToString(Result.Context->getSourceManager());
                string field_name = memberExpr->getMemberDecl()->getNameAsString();
                string field_decl_loc = memberExpr->getMemberDecl()->getBeginLoc().printToString(Result.Context->getSourceManager());
                field_decl_loc = changes(field_decl_loc);
                ...
                {
                    if (fun.variables[i].variable_name == field_name && fun.variables[i].decl_loc == field_decl_loc && fun.variables[i].using_loc == field_using_loc)
                        find = 1;
        ...
        if (auto const* memberExpr = Result.Nodes.getNodeAs<MemberExpr>("cfun_memberExpr"))
        {
            // 获取调用的是函数还是变量
            //cout << "获取类方法调用或类成员变量使用" << endl;
            string type = memberExpr->getType().getAsString();
        ...
        if (auto const* memberExpr = Result.Nodes.getNodeAs<MemberExpr>("class_memberExpr"))
        {
            // 获取调用的是函数还是变量
            //cout << "获取类方法调用或类成员变量使用" << endl;
            string type = memberExpr->getType().getAsString();
        ...
        if (auto const* callExpr = Result.Nodes.getNodeAs<CallExpr>("call_expr"))
        {
            ...
            {
                // 不是函数调用，则为调用函数指针
                string call_loc = callExpr->getBeginLoc().printToString(Result.Context->getSourceManager());
                ...
                {
                    const ImplicitCastExpr* implicitCastExpr = cast<ImplicitCastExpr>(callee);
                    if (isa<MemberExpr>(implicitCastExpr->getSubExpr()))
                    {
                        pointer_fun_name = cast<MemberExpr>(implicitCastExpr->getSubExpr())->getMemberDecl()->getNameAsString();
                        pointer_fun_type = cast<MemberExpr>(implicitCastExpr->getSubExpr())->getMemberDecl()->getType().getAsString();
                        pointer_fun_decl_loc = cast<MemberExpr>(implicitCastExpr->getSubExpr())->getMemberDecl()->getBeginLoc().printToString(Result.Context->getSourceManager());
                        pointer_fun_decl_loc = changes(pointer_fun_decl_loc);
                        auto temp = implicitCastExpr->getSubExpr();
                        auto memberExpr = cast<MemberExpr>(temp);
                        auto declRef = get_DeclRefExpr_const(memberExpr);
                        if (declRef != NULL)
                        {
                            if (isa<DeclRefExpr>(declRef))
                            {
                                ...
                                if (!isa<FunctionDecl>(func))
                                {
                                    ...
                                    struct_name = var_decl_loc + "$$" + var_name;
                                }
                            }
                        }

                    }
                    if (isa<ArraySubscriptExpr>(implicitCastExpr->getSubExpr()))
                    {
                        auto temp = cast<ArraySubscriptExpr>(implicitCastExpr->getSubExpr())->getBase();
                        if (isa<ImplicitCastExpr>(temp))
                        {
                            auto temp1 = cast<ImplicitCastExpr>(temp)->getSubExpr();
                            if (isa<MemberExpr>(temp1))
                            {
                                pointer_fun_name = cast<MemberExpr>(temp1)->getMemberDecl()->getNameAsString();
                                pointer_fun_type = cast<MemberExpr>(temp1)->getMemberDecl()->getType().getAsString();
                                pointer_fun_decl_loc = cast<MemberExpr>(temp1)->getMemberDecl()->getBeginLoc().printToString(Result.Context->getSourceManager());
                                pointer_fun_decl_loc = changes(pointer_fun_decl_loc);
                                //auto temp2 = implicitCastExpr->getSubExpr();
                ...
                if (isa<ParenExpr>(callee))
                {
                    ...
                    pointer_fun_decl_loc = temp->getBeginLoc().printToString(Result.Context->getSourceManager());
                    pointer_fun_decl_loc = changes(pointer_fun_decl_loc);
                }
                pointer_func new_pf;
                ...
                new_pf.struct_name = struct_name;
                new_pf.call_loc = call_loc;
                int find = 0;
                Function_Constraint fun = current_Fun;
                for (int i = 0; i < fun.call_pointer_func.size(); i++)
                {
                    if (fun.call_pointer_func[i].name == new_pf.name && fun.call_pointer_func[i].type == new_pf.type && fun.call_pointer_func[i].loc == new_pf.loc)
                        find = 1;
            ...
            else
            {
                ...
                if (isa<FunctionDecl>(func))
                {
                    ...
                    string CallExpr_Loc = callExpr->getSourceRange().getBegin().printToString(Result.Context->getSourceManager());
                    CallExpr_Loc = changes(CallExpr_Loc);
                    const FunctionDecl* Fun_Decl = cast<FunctionDecl>(if_func);
                    string Decl_loc = Fun_Decl->getSourceRange().getBegin().printToString(Result.Context->getSourceManager());
                    ...
                    for (int i = 0; i < fun.call_fun.size(); i++)
                    {
                        if (fun.call_fun[i].name == Fun_Call_Name && fun.call_fun[i].Decl_Loc == Decl_loc && fun.call_fun[i].CallExpr_Loc == CallExpr_Loc)
                            find = 1;
        ...
        if (auto const* binaryOperator = Result.Nodes.getNodeAs<BinaryOperator>("BinaryOperator"))
        {
            ...
            {
                const ImplicitCastExpr* implicitCastExpr = cast<ImplicitCastExpr>(binaryOperator_right);
                if (isa<DeclRefExpr>(implicitCastExpr->getSubExpr()))
                {
                    ...
                    if (isa<FunctionDecl>(func))
                    {
                        ...
                        string func_decl_loc = Fun_Call->getBeginLoc().printToString(Result.Context->getSourceManager());
                        ...
                        if (isa<MemberExpr>(binaryOperator_left) && Opp == "=")
                        {
                            ...
                            string pointer_fun_name = memberExpr->getMemberDecl()->getNameAsString();
                            string pointer_fun_type = memberExpr->getMemberDecl()->getType().getAsString();
                            string pointer_fun_decl_loc = memberExpr->getMemberDecl()->getBeginLoc().printToString(Result.Context->getSourceManager());
                            pointer_fun_decl_loc = changes(pointer_fun_decl_loc);
                            ...
                            if (declRef != NULL)
                            {
                                if (isa<DeclRefExpr>(declRef))
                                {
                                    ...
                                    if (!isa<FunctionDecl>(func))
                                    {
                                        ...
                                        string struct_name = var_decl_loc + "$$" + var_name;
                                        ...
                                        {
                                            if (pointer_func_list[i].name == pointer_fun_name && pointer_func_list[i].loc == pointer_fun_decl_loc && pointer_func_list[i].struct_name == struct_name)
                                            {
                        ...
                        else if (isa<ArraySubscriptExpr>(binaryOperator_left) && Opp == "=")
                        {
            ...
            else if (isa<UnaryOperator>(binaryOperator_right))
            {
                Expr* FS_Inc_UO_Sub = cast<UnaryOperator>(binaryOperator_right)->getSubExpr();
        ...
        if (auto const* declRef = Result.Nodes.getNodeAs<DeclRefExpr>("fun_decl_ref"))
        {
            // 获取声明
            cout << "获取函数调用或变量使用" << endl;
            auto func = declRef->getDecl();
            //string CallExpr_Loc = declRef->getSourceRange().getBegin().printToString(Result.Context->getSourceManager());
            ...
            else
            {
                ...
                for (int i = 0; i < fun.variables.size(); i++)
                {
                    if (fun.variables[i].variable_name == var_name && fun.variables[i].decl_loc == var_decl_loc && fun.variables[i].using_loc == var_using_loc)
                        find = 1;
        ...
        if (auto const* declRef = Result.Nodes.getNodeAs<DeclRefExpr>("cfun_decl_ref"))
        {
            // 获取声明
            //cout << "获取函数调用或变量使用" << endl;
            auto func = declRef->getDecl();
            ...
            else
            {
                ...
                if (find == 0)
                {
                    for (int i = 0; i < Function_list.size(); i++)
                    {
                        if (current_Fun.function_name == Function_list[i].function_name && current_Fun.function_location == Function_list[i].function_location)
                        {
                            if (var_type.find("ostream") == string::npos && var_type.find("istream") == string::npos)
                            {
                                var_using new_used_var;
                                new_used_var.variable_name = var_name;
                                new_used_var.type = var_type;
        ...
        if (auto const* declRef = Result.Nodes.getNodeAs<DeclRefExpr>("class_decl_ref"))
        {
            // 获取声明
            //cout << "获取函数调用或变量使用" << endl;
            auto func = declRef->getDecl();
            ...
            else
            {
                ...
                if (var_name != "")
                {
                    ...
                    for (int i = 0; i < cur_class.variables.size(); i++)
                    {
                        if (cur_class.variables[i].variable_name == var_name && cur_class.variables[i].decl_loc == var_decl_loc && cur_class.variables[i].using_loc == var_using_loc)
                            find = 1;
        ...
        //if (auto const* parenExpr = Result.Nodes.getNodeAs<ParenExpr>("cfun_parenExpr"))
        //{
        ...
        //if (auto const* parenExpr = Result.Nodes.getNodeAs<ParenExpr>("fun_parenExpr"))
        //{
        //    auto paren = parenExpr;
        //    string begin_loc = paren->getBeginLoc().printToString(Result.Context->getSourceManager());
        //    begin_loc = changes(begin_loc);
        ...
        //if (auto const* parenExpr = Result.Nodes.getNodeAs<ParenExpr>("class_parenExpr"))
        //{
        //    auto paren = parenExpr;
        //    string begin_loc = paren->getBeginLoc().printToString(Result.Context->getSourceManager());
        //    begin_loc = changes(begin_loc);
    ...
    unique_ptr<ASTConsumer> CreateASTConsumer(CompilerInstance& CI, StringRef file) override
    {
        ...
        auto FuncDeclMatcher =
            functionDecl(isExpansionInMainFile(),
                //anyOf(forEachDescendant(callExpr().bind("callExprFunction")), unless(forEachDescendant(callExpr().bind("callExprFunction")))),
                anyOf(forEachDescendant(declRefExpr(isExpansionInMainFile()).bind("cfun_decl_ref")), unless(forEachDescendant(declRefExpr(isExpansionInMainFile()).bind("cfun_decl_ref")))),
                anyOf(forEachDescendant(memberExpr(isExpansionInMainFile()).bind("cfun_memberExpr")), unless(forEachDescendant(memberExpr(isExpansionInMainFile()).bind("cfun_memberExpr"))))
                //anyOf(forEachDescendant(parenExpr(isExpansionInMainFile()).bind("cfun_parenExpr")), unless(forEachDescendant(parenExpr(isExpansionInMainFile()).bind("cfun_parenExpr"))))
            )
            .bind("FunctiondFeclWithCall");

        auto refExpr = declRefExpr(isExpansionInMainFile()).bind("declRefExpr");

        auto classMatcher = cxxRecordDecl(isExpansionInMainFile(), 
            anyOf(forEachDescendant(declRefExpr(isExpansionInMainFile()).bind("class_decl_ref")), unless(forEachDescendant(declRefExpr(isExpansionInMainFile()).bind("class_decl_ref")))),
            anyOf(forEachDescendant(memberExpr(isExpansionInMainFile()).bind("class_memberExpr")), unless(forEachDescendant(memberExpr(isExpansionInMainFile()).bind("class_memberExpr"))))
            //anyOf(forEachDescendant(parenExpr(isExpansionInMainFile()).bind("cfun_parenExpr")), unless(forEachDescendant(parenExpr(isExpansionInMainFile()).bind("class_parenExpr"))))
        ).bind("CXXRecordDecl");

        auto MemberCallExpr = cxxMemberCallExpr(isExpansionInMainFile()).bind("CXXMemberCallExpr");

        auto ConstructorDecl = cxxConstructorDecl(isExpansionInMainFile()).bind("CXXConstructorDecl");

        auto mmemberExpr = memberExpr(isExpansionInMainFile()).bind("MemberExpr");//使用成员变量或者成员方法

        DeclarationMatcher GlobalVarMatcher = varDecl(isExpansionInMainFile(),
            hasGlobalStorage(), unless(hasAncestor(functionDecl()))

        ).bind("gvar");

        auto MethodDecl = cxxMethodDecl(isExpansionInMainFile(), 
            anyOf(forEachDescendant(declRefExpr(isExpansionInMainFile()).bind("fun_decl_ref")), unless(forEachDescendant(declRefExpr(isExpansionInMainFile()).bind("fun_decl_ref")))),
            anyOf(forEachDescendant(memberExpr(isExpansionInMainFile()).bind("fun_memberExpr")), unless(forEachDescendant(memberExpr(isExpansionInMainFile()).bind("fun_memberExpr"))))
            //anyOf(forEachDescendant(parenExpr(isExpansionInMainFile()).bind("fun_parenExpr")), unless(forEachDescendant(parenExpr(isExpansionInMainFile()).bind("fun_parenExpr"))))
        ).bind("CXXMethodDecl");

        auto callexpr = callExpr(isExpansionInMainFile()).bind("call_expr");
        auto binaryOperator_ = binaryOperator(isExpansionInMainFile()).bind("BinaryOperator");
...
string print_class_info()
{
    ...
    for (int i = 0; i < Class_list.size(); i++)
    {
        ...
        for (int j = 0; j < temp_class.Member_variables.size(); j++)
        {
            ...
            info += "[";
            info += m_var.variable_name + " ;; " + m_var.decl_loc + " ;; " + m_var.decl_class + " ;; " + m_var.type_decl_loc+ "$$" + m_var.type + " ;; "+ m_var.authority + "] ;; ";
        }
        info += "} ## ";
        ...
        for (int j = 0; j < temp_class.Functions.size(); j++)
        {
            ...
            info += "[";
            info += m_fun.function_name + " ;; " + m_fun.function_location + " ;; " + m_fun.calss + " ;; " + to_string(m_fun.if_static) + " ;; " + to_string(m_fun.if_virtual) + "] ;; ";
        }
        info += "}}\n";
...
string print_fun_var_info() {
    ...
    for (int i = 0; i < Function_list.size(); i++)
    {
        ...
        for (int j = 0; j < Function_list[i].variables.size(); j++)
        {
            ...
            info += Function_list[i].variables[j].type_decl_loc + "$$" + Function_list[i].variables[j].type + "@@";
            info += Function_list[i].variables[j].using_loc + "@@";
            info += Function_list[i].variables[j].decl_loc + "@@";
            info += Function_list[i].variables[j].class_decl_loc + "$$" + Function_list[i].variables[j].decl_class + "];;";
        }
        info += "\n";
    }
...
string print_class_var_info() {
    ...
    for (int i = 0; i < Class_list.size(); i++)
    {
        ...
        for (int j = 0; j < Class_list[i].variables.size(); j++)
        {
            ...
            info += Class_list[i].variables[j].type_decl_loc + "$$" + Class_list[i].variables[j].type + "@@";//类型
            info += Class_list[i].variables[j].using_loc + "@@";//使用位置
            info += Class_list[i].variables[j].decl_loc + "@@";//定义位置
            info += Class_list[i].variables[j].class_decl_loc + "$$" + Class_list[i].variables[j].decl_class + "];;";//类
        }
        info += "\n";
    }
...
string printinfo()
{
    ...
    {
        info += "{";
        info += "[" + Function_list[i].function_location + " ;; " + Function_list[i].isbody + " ;; " + Function_list[i].function_name + " ;; "+ Function_list[i].calss + " ;; " + to_string(Function_list[i].fun_type) + " ;; " + to_string(Function_list[i].if_static) + " ;; " + Function_list[i].authority +" ;; "+ Function_list[i].start+" ;; "+ Function_list[i].end + "], ";
        for (int j = 0; j < Function_list[i].call_fun.size(); j++)
        {
            Call_Fun_Info call_fun = Function_list[i].call_fun[j];
            info += "[" + call_fun.Decl_Loc + " ;; " + call_fun.CallExpr_Loc + " ;; " + call_fun.name + "], ";
        }

        for (int j = 0; j < Function_list[i].call_pointer_func.size(); j++)
        {
            pointer_func pf = Function_list[i].call_pointer_func[j];
            if (pf.struct_name != "")
                info += "[" + pf.loc + " ;; " + pf.call_loc + " ;; " + pf.struct_name + "->" + pf.name + "], ";
            else
                info += "[" + pf.loc + " ;; " + pf.call_loc + " ;; " + pf.name + "], ";
        }
        info += "}\n\n";
    }
    return info;
...
string printinfo_param()
{
    ...
    for (int i = 0; i < Function_list.size(); i++)
    {
        ...
        for (int j = 0; j < params.size(); j++) {
            info += params[j].type_decl_loc + "$$" + params[j].type + ";" + params[j].name + ";";
        }
        info += "\n";
    }
...
string printinfo_param1()
{
    ...
    for (int i = 0; i < Function_list.size(); i++)
    {
        ...
        for (int j = 0; j < Function_list[i].params.size(); j++)
        {
            ...
            info += Function_list[i].params[j].type_decl_loc + "$$" + Function_list[i].params[j].type + "];;";
        }
        info += "\n";
    }
...
string printinfo_name()
{
    ...
    for (int i = 0; i < Function_list.size(); i++)
    {
        ...
        info += Function_list[i].function_location+ " " + Function_list[i].function_name + " " + Function_list[i].start + " " + Function_list[i].end + "\n";
    }
    return info;
}
...
string printpf_info()
{
    ...
    {
        info += "{";
        info += "[" + pointer_func_list[i].loc + " ;; " + pointer_func_list[i].struct_name + "->" + pointer_func_list[i].name + "], ";
...
int main(int argc, const char** argv)
{
    ...
    //const char* temp[] = { "CPP_Parsing.exe","func_pointer.c" };
    string file_temp = "";
    //cout << argc << endl;
